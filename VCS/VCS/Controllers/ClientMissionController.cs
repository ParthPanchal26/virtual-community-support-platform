...
namespace VCS.Controllers {
    ...
    public class ClientMissionController(IMissionService missionService) : ControllerBase {
        ...
        public async Task<IActionResult> GetAllClientMissions() {
            try {
                var missions = await _missionService.GetAllClientMissions();
                return Ok(new ResponseResult() { Data = missions, Message = string.Empty, Result = ResponseStatus.Success });
            } catch {
                return BadRequest(new ResponseResult() { Data = null, Message = "Error in fetching all missions.", Result = ResponseStatus.Error });
            }
        }
